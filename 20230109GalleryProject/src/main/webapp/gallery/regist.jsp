...
<script>
// 이미지 미리보기
// 매개변수로 유저가 선택한 파일 정보가 넘어옴
function preview(file) { 
	console.log("넘겨받은 파일 정보" + file);
	// 자바 스크립트로 파일에 대한 스트림 객체가 지원된다
	...
	$(function() {
		...
		$("input[name='file']").on("change", function(e) {
			...
			preview(this.files[0]); // 미리보기 구현 함수에게 파일 정보 전달 
		});
	...
	<form>
		<table>
			...
			<%if(member==null) { %>
				...
				<%} else{ %>
				<td><input type="text" value="<%=member.getName() %>" name="writer"></td>
				<%} %>
			</tr>
			<tr>
